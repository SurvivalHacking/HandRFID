...
#define FONT_TITLE  &FreeSansBold12pt7b   // titoli / header
#define FONT_BODY   &FreeSans9pt7b        // testo normale
#define FONT_SMALL  &FreeSans9pt7b        // testo piccolo (status, versione)
...
void showStatus(const char* msg, uint16_t color) {
  ...
  tft.drawString(msg, SCREEN_W / 2, SCREEN_H - 14);
}
...
void drawReadResult(bool showBack) {
  ...
  tft.drawString(mfgMatLabel, cx, 44);
...
void drawWriteSelectMfg() {
  ...
  tft.setTextDatum(MC_DATUM);
  setFontTitle();
  tft.drawString(strManufacturer[curLang], SCREEN_W / 2, 16);
...
void drawWriteSelectMat() {
  ...
  tft.setTextDatum(MC_DATUM);
  setFontTitle();
  tft.drawString(strSelectMaterial[curLang], SCREEN_W / 2, 16);
...
void drawWriteSelectColor() {
  ...
  setFontTitle();
  tft.setTextColor(TFT_CYAN, TFT_BLACK);
  tft.drawString(strSelectColor[curLang], SCREEN_W / 2, 10);
  setFontSmall();
  tft.setTextColor(TFT_YELLOW, TFT_BLACK);
  tft.drawString(qidiMatCodes[selMatIdx].name, SCREEN_W / 2, 26);
...
bool readTagAuto() {
  ...
  if (readTagValidated()) {
    ...
    if (!tagPresent) {
      ...
      setFontTitle();
      tft.drawString(curLang == LANG_IT ? "TAG SCONOSCIUTO" : "UNKNOWN TAG", SCREEN_W / 2, SCREEN_H / 2 - 20);
      setFontBody();
      tft.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
      tft.drawString(curLang == LANG_IT ? "Dati non validi o tag vuoto" : "Invalid data or blank tag", SCREEN_W / 2, SCREEN_H / 2 + 10);
      Btn btnBack = {10, SCREEN_H - 40, SCREEN_W - 20, BTN_H, strBack[curLang], TFT_NAVY};
      drawBtn(btnBack);
      ledOff(); ledFlashErr(); ledOff();
      return false;
  ...
  if (!hasValidUID) {
    ...
    tft.setTextColor(TFT_RED, TFT_BLACK);
    setFontTitle();
    tft.drawString(curLang == LANG_IT ? "TAG NON RICONOSCIUTO" : "UNKNOWN TAG", SCREEN_W / 2, SCREEN_H / 2 - 20);
    setFontBody();
    tft.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
    tft.drawString(curLang == LANG_IT ? "Tipo tag non supportato" : "Unsupported tag type", SCREEN_W / 2, SCREEN_H / 2 + 10);
    Btn btnBack = {10, SCREEN_H - 40, SCREEN_W - 20, BTN_H, strBack[curLang], TFT_NAVY};
    drawBtn(btnBack);
    ledOff(); ledFlashErr(); ledOff();
  }
...
void drawWriteSelectMode() {
  ...
  tft.setTextDatum(MC_DATUM);
  setFontTitle();
  tft.drawString(strSelectMode[curLang], SCREEN_W / 2, 16);
...
void drawAceSelectColor() {
  ...
  setFontSmall();
  tft.setTextColor(TFT_YELLOW, TFT_BLACK);
  tft.drawString(aceMatList[selAceMatIdx].name, SCREEN_W / 2, 26);
...
void drawAceSelectWeight() {
  ...
  setFontTitle();
  tft.setTextColor(TFT_CYAN, TFT_BLACK);
  tft.drawString(strSelectWeight[curLang], SCREEN_W / 2, 10);
  ...
  tft.drawString(aceMatList[selAceMatIdx].name, SCREEN_W / 2 - 14, sumY);
  ...
  tft.setTextDatum(ML_DATUM);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString(colName, SCREEN_W / 2 + 10, sumY);
  tft.setTextDatum(MC_DATUM);
...
void drawReadResultAnycubic(bool showBack) {
  ...
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  setFontTitle();
  tft.drawString(mfgName + " " + typeName, cx, 44);
  ...
  if (aceReadSku.length()) {
    tft.drawString("SKU: " + aceReadSku, 10, y); y += 20;
  }
...
void drawSetupScreen() {
  ...
  tft.setTextDatum(MC_DATUM);
  setFontTitle();
  tft.drawString(strSetup[curLang], SCREEN_W / 2, 16);
...
void drawLangSelect() {
  ...
  tft.setTextDatum(MC_DATUM);
  setFontTitle();
  tft.drawString(strSelectLang[curLang], SCREEN_W / 2, 16);
...
void drawCalibScreen() {
  ...
  tft.setTextDatum(MC_DATUM);
  setFontBody();
  tft.drawString(strCalibInstr[curLang], SCREEN_W / 2, SCREEN_H / 2);
...
void runCalibration() {
  ...
  for (int pt = 0; pt < 4; pt++) {
    ...
    tft.setTextDatum(MC_DATUM);
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
    tft.drawString(progStr, SCREEN_W / 2, SCREEN_H / 2 + 20);
...
void drawNotice() {
  ...
  tft.setTextDatum(MC_DATUM);
  setFontTitle();
  tft.drawString(strPleaseNote[curLang], SCREEN_W / 2, 16);
  ...
  while (idx < (int)noticeText.length() && y < SCREEN_H - 50) {
    ...
    tft.drawString(line, SCREEN_W / 2, y);
    y += 16;
  }
...
void drawConfirm() {
  ...
  tft.setTextDatum(MC_DATUM);
  setFontTitle();
  tft.drawString(strConfirm[curLang], SCREEN_W / 2, 16);
...
void drawWriteDone() {
  ...
  tft.setTextDatum(MC_DATUM);
  setFontTitle();
  tft.drawString(strWriteDoneTitle[curLang], SCREEN_W / 2, 24);
...
void showQRScreen(const char* title, const char* url, const char* subtitle) {
  ...
  setFontTitle();
  tft.setTextColor(TFT_CYAN, TFT_BLACK);
  tft.drawString(title, SCREEN_W / 2, 18);
  ...
  setFontBody();
  tft.setTextColor(TFT_GREEN, TFT_BLACK);
  tft.drawString(subtitle, SCREEN_W / 2, subY);
}
