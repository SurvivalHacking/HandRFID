...
//
//  V1.2C 05/06/2022 by Davide Gatti
//  Aggiunta lingua Francese
//  Versione dedicata per il display CYD Clone ESP32-024  https://shorturl.at/3kmK1
//
// ============================================================
...
#ifdef DISPLAY_24C
    // Clone: calibMinX/MaxX=rawY, calibMinY/MaxY=rawX, entrambi specchiati
    int tx = map(p.y, calibMinX, calibMaxX, 0, SCREEN_W);
    int ty = map(p.x, calibMinY, calibMaxY, 0, SCREEN_H);
    x = constrain(SCREEN_W - 1 - tx, 0, SCREEN_W - 1);
    y = constrain(SCREEN_H - 1 - ty, 0, SCREEN_H - 1);
#else
    int tx = map(p.x, calibMinX, calibMaxX, 0, SCREEN_W);
    int ty = map(p.y, calibMinY, calibMaxY, 0, SCREEN_H);
...
void runCalibration() {
  ...
  for (int pt = 0; pt < 4; pt++) {
    ...
    // Wait for stable touch — legge direttamente ts.getPoint()
    unsigned long touchStart = 0;
    bool touching = false;
    while (true) {
      TS_Point p = ts.getPoint();
      bool isTouched = (p.z > 50);
      if (isTouched) {
        if (!touching) {
          touchStart = millis();
          touching = true;
        ...
        if (millis() - touchStart > 800) {
          ...
          rawX[pt] = sumX / 8;
          rawY[pt] = sumY / 8;
          Serial.printf("Punto %d: rawX=%d rawY=%d\n", pt, rawX[pt], rawY[pt]);
          Serial.flush();
          break;
        }
      } else {
    ...
    // Wait for release
    while (ts.getPoint().z > 50) delay(10);
...
void setup() {
  ...
  /*
  // Forza ricalibrazione - rimuovere dopo
  prefs.begin(NVS_NAMESPACE, false);
  prefs.putBool("hascal", false);
  prefs.end();
  hasCalib = false;
  */
...
void loop() {
  ...
  static bool calibPrinted = false;
  if (!calibPrinted) {
    Serial.printf("CALIB: minX=%d maxX=%d minY=%d maxY=%d\n", calibMinX, calibMaxX, calibMinY, calibMaxY);
    calibPrinted = true;
  }
...
if (ts.touched()) {
  ...
  int tx = map(p.x, calibMinX, calibMaxX, 0, SCREEN_W);
  int ty = map(p.y, calibMinY, calibMaxY, 0, SCREEN_H);
  int fx = constrain(SCREEN_W - 1 - ty, 0, SCREEN_W - 1);
  int fy = constrain(tx, 0, SCREEN_H - 1);
  Serial.printf("RAW x=%d y=%d | mapped tx=%d ty=%d | final x=%d y=%d\n", 
                p.x, p.y, tx, ty, fx, fy);
}
*/
